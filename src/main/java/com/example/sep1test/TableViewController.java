...
public class TableViewController implements Initializable
{
  ...
  private void clickRemove(ActionEvent e) throws IOException {
    ...
    Integer[] selectIndices = new Integer[list.size()];
    selectIndices = list.toArray(selectIndices);

    Arrays.sort(selectIndices);

    for (int i = selectIndices.length - 1; i >= 0; i--){
      selectionModel.clearSelection(selectIndices[i].intValue());
      resTable.getItems().remove(selectIndices[i].intValue());
    }
  }
